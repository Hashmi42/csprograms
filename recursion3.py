    ...
    if start>=len(nums):
        ...
        if start != len(nums) - 1:
            while nums[index] == nums[index + 1]:
                ...
                if index == len(nums) - 1:
                    break
        return groupSumClump(start + count, nums, target) or groupSumClump(start + count, nums,
                                                                           target - count * nums[start])
...
def splitArrayHelper(start, nums, num1, num2):
  if start>=len(nums):
    return num1==num2
  if splitArrayHelper(start+1, nums, num1 + nums[start], num2) or splitArrayHelper(start+1, nums, num1, num2 + nums[start]):
    return True
  return False
...
def splitOdd10Helper(start, nums, num1, num2):
    if start>=len(nums):
        return num1%10==0 and num2%2==1
    if splitOdd10Helper(start+1, nums, num1 + nums[start], num2) or splitOdd10Helper(start+1, nums, num1, num2 + nums[start]):
        return True
    return False
...
def split53Helper(start, nums, num1, num2):
  if start>=len(nums):
    return num1==num2
  if nums[start]%5==0:
    return split53Helper(start+1, nums, num1 + nums[start], num2)
  elif nums[start]%3==0:
    return split53Helper(start+1, nums, num1, num2 + nums[start])
  else:
    if split53Helper(start+1, nums, num1 + nums[start], num2) or split53Helper(start+1, nums, num1, num2 + nums[start]):
      return True
  return False
