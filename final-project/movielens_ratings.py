...
# passes each piece of data through a transformation creating a new RDD w/ the results
rdd = lines.map(parse_line) # movie_id, rating
#print_rdd(rdd, "movie_rating_pairs")

# similar to map; however, works because the data is in pairs
...
# groups ratings by movie and adds them 
...
# creates a new RDD with the additional ratings
...
# creates a new RDD with existing IMDB data
rdd_links = links_lines.map(parse_links_line) # movie_id, imdb_id
#print_rdd(rdd_links, "rdd_links")

# joins our movielens RDD w/ the rdd_links created above
...
# add imdb_id prefix () 
# creates new RDD of joined dataframe
